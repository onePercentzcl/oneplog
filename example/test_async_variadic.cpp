/**
 * @file test_async_variadic.cpp
 * @brief Test with variadic template like FastLogger
 */
...
// Test with variadic template
template<bool EnableWFC = false, bool EnableShadowTail = false>
class VariadicFastLogger {
public:
    VariadicFastLogger() : m_sink{} { 
...
int main() {
    std::cout << "Testing VariadicFastLogger (with variadic template)...\n";
    
    for (int run = 0; run < 20; ++run) {
        std::cout << "Run " << run << " starting...\n";
        
        {
            VariadicFastLogger<false, false> logger;
