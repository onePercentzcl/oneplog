 ...
 * @brief Shared Memory Layout Definition
 * @brief 共享内存布局定义
 *
...
/**
 * @brief Header describing the shared memory layout.
 * @brief 描述共享内存布局的头部。
 * * @details Located at the very beginning of the shared memory block.
 * @details 位于共享内存块的最开始位置。
 */
struct alignas(kCacheLineSize) ShmHeader {
    ...
    uint64_t m_totalSize{};             ///< Total size of SHM / 共享内存总大小
...
/**
 * @brief Layout descriptor for the entire shared memory block.
 * @brief 整个共享内存块的布局描述符。
...
private:
    ...
    ShmHeader m_shmHeader{}; ///< 0x00: Header
