...
onePlog 是一个高性能 C++20 多进程聚合日志系统，设计目标是提供零拷贝、低延迟的日志记录能力。系统支持操作模式：
...
graph TB
    ...
    subgraph "传输层 - 多进程模式"
        ...
        NameTable[ProcessThreadNameTable<br/>名称-ID对照表]
        ConfigTable[LoggerConfigTable<br/>配置表]
    end
    ...
    SharedMemory --> NameTable
    SharedMemory --> ConfigTable
...
| 共享内存 | 无 | 无 | SharedMemory (含 RingBuffer + NameTable + ConfigTable) |
...
### 5. SharedRingBuffer（共享内存环形队列）

SharedRingBuffer 是共享内存中的环形队列，与 HeapRingBuffer 的主要区别在于存储位置。SharedRingBuffer 的数据存储在共享内存中，可以被多个进程访问。

#### 共享内存整体布局

```
System Shared Memory (系统共享内存)
└── Logger Shared Memory (Logger 共享内存，通过 Offset 定位)
    ├── Metadata (元数据)
    ├── Logger Config (配置参数)
    ├── ProcessThreadNameTable (进程/线程名称-ID对照表)
    └── SharedRingBuffer (共享环形缓冲区)
        ├── Header (管理头)
        │   ├── Read Index (原子变量，独占缓存行)
        │   ├── Write Index (原子变量，独占缓存行)
        │   └── Consumer State (消费者状态标志)
        └── Slots[N] (数据槽位，N 为 2 的幂)
            └── Slot
                ├── WFC Flag (WFC 标志)
                ├── Slot State (槽位状态)
                └── Data (日志数据)
...
在多进程模式下：
- 如果初始化时没有传入现有的共享内存，Logger 会按照默认参数新建一块
- 用户也可以自定义新建共享内存的配置参数
- 子进程通过 Offset 连接到已存在的 Logger 共享内存

#### SharedRingBuffer 设计要点

1. **管理头 (Header)**：
    - Read Index 和 Write Index 使用原子变量
    - 原子变量独占缓存行，避免伪共享
    - 包含消费者状态标志（用于通知优化）

2. **数据槽位 (Slots)**：
    - 槽位大小为 Cache Line 大小的整数倍
    - 槽位数量 N 为 2 的幂（方便位运算取模）
    - 每个槽位包含：WFC 标志、槽位状态、数据区域
...
struct SharedMemoryMetadata {
    uint32_t magic;           // 魔数，用于验证 (0x4F4E4550 = "ONEP")
    uint32_t version;         // 版本号
    size_t totalSize;         // Logger 共享内存总大小
    size_t configOffset;      // 配置参数偏移
    size_t nameTableOffset;   // 名称表偏移
    size_t ringBufferOffset;  // RingBuffer 偏移
};
...
struct alignas(kCacheLineSize) SharedLoggerConfig {
    std::atomic<Level> logLevel{Level::Info};
...
// ============================================================================
// SharedRingBuffer 管理头
// ============================================================================

struct alignas(kCacheLineSize) SharedRingBufferHeader {
    // Read Index（独占缓存行）
    alignas(kCacheLineSize) std::atomic<size_t> readIndex{0};
    
    // Write Index（独占缓存行）
    alignas(kCacheLineSize) std::atomic<size_t> writeIndex{0};
    ...
    // 配置信息
    size_t capacity;          // 槽位数量（2 的幂）
    size_t slotSize;          // 每个槽位大小（Cache Line 的整数倍）
    QueueFullPolicy policy;   // 队列满策略
};

// ============================================================================
// 槽位结构（大小为 Cache Line 的整数倍）
// ============================================================================

struct alignas(kCacheLineSize) SharedSlot {
    // 槽位控制信息
    std::atomic<SlotState> state{SlotState::Empty};  // 槽位状态
    std::atomic<WFCState> wfc{kWFCNone};             // WFC 标志
    
    // 数据区域（LogEntry）
    // 注意：实际大小根据 LogEntry 大小调整，确保整个槽位是 Cache Line 的整数倍
    uint8_t data[sizeof(LogEntry)];
    
    // 填充到 Cache Line 的整数倍
    // padding 大小 = (ceil((sizeof(state) + sizeof(wfc) + sizeof(data)) / kCacheLineSize) * kCacheLineSize) - 实际大小
};

// ============================================================================
// SharedRingBuffer 类（继承自 RingBufferBase）
// ============================================================================

/**
 * SharedRingBuffer 继承自 RingBufferBase，与 HeapRingBuffer 的主要区别：
 * - HeapRingBuffer：数据存储在堆内存中，由 std::vector 管理
 * - SharedRingBuffer：数据存储在共享内存中，由外部传入的内存指针管理
 * 
 * 两者共享 RingBufferBase 的核心逻辑：
 * - AcquireSlot/CommitSlot
 * - TryPush/TryPushWFC
 * - TryPop/TryPopBatch
 * - WFC 支持方法
 * - 队列满策略
...
public:
    ...
    // 在共享内存中创建（主进程调用）
    static SharedRingBuffer* Create(void* memory, size_t capacity, 
                                    QueueFullPolicy policy = QueueFullPolicy::DropNewest);
    
    // 连接到已存在的共享内存（子进程调用）
    static SharedRingBuffer* Connect(void* memory);
    
    // 计算所需的共享内存大小
    static size_t CalculateRequiredSize(size_t capacity);
    
    // 继承自 RingBufferBase 的方法：
    // - AcquireSlot/CommitSlot
    // - TryPush/TryPushWFC
    // - TryPop/TryPopBatch
    // - WFC 支持方法
...
private:
    SharedRingBuffer(void* memory, size_t capacity, bool isOwner,
                     QueueFullPolicy policy = QueueFullPolicy::DropNewest);
    
    bool m_isOwner{false};  // 是否是创建者（主进程）
    
    // 注意：SharedRingBuffer 不使用 std::vector，而是直接使用共享内存中的指针
    // 基类的 m_buffer 和 m_slotStatus 需要指向共享内存中的位置
};
...
public:
    ...
    // 获取组件
    SharedLoggerConfig* GetConfig();
    ProcessThreadNameTable* GetNameTable();
    SharedRingBuffer<LogEntry>* GetRingBuffer();
    ...
    // 通知机制（fork 用 EventFD，非 fork 用 UDS）
    void NotifyConsumer();
...
3. 添加进程 ID 到日志条目
4. 将转换后的数据写入 SharedMemory 中的 SharedRingBuffer
...
private:
    ...
    SharedMemory& m_sharedMemory;  // 使用 SharedMemory 而不是直接使用 SharedRingBuffer
    std::thread m_thread;
